...
def ChiSquared_CDM(params, data, A, T, P):
    """
    ChiSquared_CDM(params, data)

    Calculate the Chi-Square value of the data for the Circular Diffusion Model (CDM).
    ...
    if int((min(RT) - t0 + st / 2) / dt) <= 0:
        return np.inf  # Chi-Square should be high for invalid inputs
    ...
    for i in range(len(CA)):
        ...
        if Z.size == 0 or np.sum(P_) == 0:
            return np.inf  # Chi-Square should be high for invalid outputs

        expected = np.sum(Z * P_)
        observed = np.sum(P_)

        chi_sq += ((observed - expected)**2) / (expected + 1e-10)

    return chi_sq

def optimize_cdm_chi_squared(params_list, datasets, A, T, P, bounds, maxiter=1000, popsize=15):
    """
    Optimize the parameters of the Circular Diffusion Model (CDM) using Chi-Square as the objective function.
    ...
    for i, initial_params in tqdm(enumerate(params_list), total=len(params_list), desc="Optimizing parameter sets (Chi-Square)"):
        param_results = {}

        for n_trials, data in tqdm(datasets[i].items(), desc=f"Parameter set {i+1}", leave=False):
            # Perform Chi-Square optimization
            chi_squared_result = differential_evolution(
                lambda params: ChiSquared_CDM(params, data, A, T, P),
                bounds,
                maxiter=maxiter,
                popsize=popsize,
            ...
            param_results[n_trials] = chi_squared_result
